...
def _pcca_connected_isa(evec, n_clusters):
    ...
    """
    logger.info("_pcca_connected_isa function entering")
    (n, m) = evec.shape
    ...
    #print "chi \n ", chi
    logger.info("_pcca_connected_isa function exiting")
...
def _opt_soft(eigvectors, rot_matrix, n_clusters):
    ...
    """
    logger.info("_opt_soft function entering")
    ...
    logger.info("scipy.optimize.fmin on susanna_func call")
    rot_crop_vec_opt = fmin(susanna_func, rot_crop_vec, args=(eigvectors,), disp=False)
    logger.info("scipy.optimize.fmin on susanna_func return")

    rot_crop_matrix = np.reshape(rot_crop_vec_opt, (x, y))
    rot_matrix = _fill_matrix(rot_crop_matrix, eigvectors)

    logger.info("_opt_soft function exiting")
    return rot_matrix
...
def _pcca_connected(P, n, return_rot=False):
    ...
    [1] S. Roeblitz and M. Weber, Fuzzy spectral clustering by PCCA+:
        ...
        Adv Data Anal Classif 7, 147-179 (2013).
    """
    logger.info("_pcca_connected function entering")
    ...
    logger.info("_pcca_connected_isa function call")
    (chi, rot_matrix) = _pcca_connected_isa(evecs, n)
    logger.info("_pcca_connected_isa function return")

    #print "initial chi = \n",chi

    # optimize the rotation matrix with PCCA++.
    logger.info("_opt_soft function call")
    rot_matrix = _opt_soft(evecs, rot_matrix, n)
    logger.info("_opt_soft function return")
    ...
    logger.info("_pcca_connected function exiting")
    return memberships
...
def pcca(P, m):
    ...
    logger.info("pcca function entering")
    ...
    logger.info("pcca function n_closed_components: %d"%n_closed_components)
    ...
    logger.info("pcca function exiting")
...
class PCCA(object):
    ...
    def __init__(self, P, m):
        ...
        logger.info("pcca function call")
        self._M = pcca(P, m)
        logger.info("pcca function returns")
